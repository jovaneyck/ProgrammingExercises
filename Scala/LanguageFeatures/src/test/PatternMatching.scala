...
class PatternMatching extends TestBase{
	...
	it should "support wildcards" in {
	  def valueOf(t: Tree) = t match {
	    case NonEmpty(value, _,_) => value //_ can be used as a wildcard
	    case _ => throw new IllegalArgumentException("value of an empty node") //even for composite data structures!
	  }
